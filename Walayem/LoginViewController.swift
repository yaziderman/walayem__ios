...
class LoginViewController: UIViewController, UITextFieldDelegate, GIDSignInDelegate {
    ...
    @IBAction func login(_ sender: UIButton) {
        ...
        OdooClient.sharedInstance().authenticate(email: email, password: password, database: WalayemApi.DB_NAME) { (result, error) in
            ...
            self.loadUserDetails(result)
        }
    }
    
    func saveLocation(_ data: [String: Any]) {
        UserDefaults.standard.removeObject(forKey: UserDefaultsKeys.CHEF_LOCATION)
        guard let item = data["location"] as? [String:Any] else {
            return
        }
        guard let chefLocation = ChefLocation(dict: item) else {
            return
        }
        chefLocation.saveToUserDefaults()
    }
    
    func saveAreaCoverage(_ data: [String: Any]) {
        UserDefaults.standard.removeObject(forKey: UserDefaultsKeys.CHEF_COVERAGE_AREA)
        guard let array = data["coverage"] as? [[String:Any]] else {
            return
        }
        guard let chefCoverage = ChefAreaCoverage(dictArray: array) else {
            return
        }
