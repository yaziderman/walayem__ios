...
class UpdateProfileViewController: UITableViewController, ChefCoverageAreaDelegate, LocationSelectionDelegate {
	...
	func updateProfile() {
		...
		params["chef_description"] = user?.chefDescription
	...
	private func updateUI(){
		...
		let isChef = UserDefaults.standard.bool(forKey: UserDefaultsKeys.IS_CHEF)
		if isChef {
			descriptionLabel.text = UserDefaults.standard.string(forKey: UserDefaultsKeys.CHEF_DESCRIPTION)
	...
	override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		if let _ = UserDefaults.standard.string(forKey: UserDefaultsKeys.AUTH_TOKEN), indexPath.row == 5{
			return 0
		}
		if indexPath.row == 3{
			return 0
		...
		let isChef = UserDefaults.standard.bool(forKey: UserDefaultsKeys.IS_CHEF)
		if !isChef && (indexPath.row == 4 || indexPath.row == 6 || indexPath.row == 7) {
			return 0
		}
		
		if isChef && indexPath.row == 4 {
...
//		let isChef = UserDefaults.standard.bool(forKey: UserDefaultsKeys.IS_CHEF)
//		if isChef && indexPath.row == 4 {
//			cell.detailTextLabel?.text = UserDefaults.standard.string(forKey: UserDefaultsKeys.CHEF_DESCRIPTION)
//		}
//	}
	...
	override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
		...
		switch segue.identifier ?? ""{
			...
			case "ChangeDescriptionVCSegue":
				guard let destinationVC = segue.destination as? ChangeDescriptionViewController else {
					fatalError("Unexpected destination \(segue.identifier)")
				}
				destinationVC.descriptionText = user?.chefDescription ?? ""
				destinationVC.descriptionChangedProtocol = self
...
extension UpdateProfileViewController: DescriptionChangedProtocol {
	
	func onDescriptionChanged(description: String) {
		descriptionLabel.text = description
		user?.chefDescription = description
		updateDescription(description: description)
	}
	
	func updateDescription(description: String) {
		
		let params = ["method":"write","model":"res.partner","args":[[user?.partner_id],["chef_description": description]], "kwargs": [:]] as [String : Any]
		
		RestClient().request("http://app.walayem.com/web/dataset/call_kw/res.partner/write", params, self) { (result, error) in
			if let error = error {
				let errmsg = error.userInfo[NSLocalizedDescriptionKey] as? String
				self.showMessagePrompt(title: "Error", message: errmsg ?? error.localizedDescription)
				return
			...
			UserDefaults.standard.set(description, forKey: UserDefaultsKeys.CHEF_DESCRIPTION)
